...
const MYID = new ObjectId('6759eb34c6ccd1db34c79407')
...
const insertMultipleTweets = async (ids: ObjectId[]) => {
  ...
  let count = 0
  const result = await Promise.all(
    ids.map(async (_id, index) => {
      await Promise.all([insertTweet(_id, createRandomTweet()), insertTweet(_id, createRandomTweet())])
      count += 2
